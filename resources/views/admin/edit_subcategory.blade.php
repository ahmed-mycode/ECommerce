                ...
                <div class="row justify-content-center">
                    <div class="col-md-8">
                        <div class="card">
                            ...
                            @endif
                            <div class="card-header">{{__('admin/category.update.subcategory')}}</div>

                            <div class="card-body">
                                <form method="post" action="{{route('admin.update.subcategory', $subCategory->id)}}" novalidate>
                                    @csrf
                                            ...
                                            <select name="parent_id" class="select2 form-control">
                                                <optgroup label="{{__('admin/category.select.category.name')}}">
                                                    @foreach($all_categories as $category)
                                                        <option value="{{$category -> id }}" @if($category->id == $subCategory->parent_id) selected @endif>
                                                            {{$category -> name}}
                                                        </option>
                                                    @endforeach
                                                </optgroup>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-6">
                                            <input id="name" type="text" class="form-control" name="slug"
                                                   value="{{$subCategory->slug}}" required autocomplete="name" autofocus>
                                            @error('slug')
                                            <span class="text-danger">{{$message}}</span>
                                            @enderror
                                        </div>
